...
frame_probe(GstPad* pad, GstPadProbeInfo* info, gpointer user_data) {
    if (info->type & GST_PAD_PROBE_TYPE_BUFFER) {
        ...
        if (frame_count % 10 == 0) {  // 10프레임마다 출력
            ...
            std::cout << "[INFO] Frame " << frame_count.load() 
...
int main(int argc, char* argv[]) {
    ...
    // RTSP 소스 설정 (연결 안정성 개선)
    g_object_set(G_OBJECT(rtspsrc), 
                 "location", uri.c_str(),
                 "protocols", 0x00000004,  // TCP only
                 "latency", 0,             // 지연시간 최소화
                 "buffer-mode", 1,         // 버퍼 모드 (auto)
                 "connection-speed", 1000, // 연결 속도 (kbps)
                 "timeout", 60000000,      // 60초 타임아웃 (더 길게)
                 "tcp-timeout", 60000000,
                 "retry", 10,              // 재시도 횟수 증가
                 "drop-on-latency", FALSE, // 지연 시 드롭 비활성화
                 NULL);
    ...
    std::thread stats_thread([&]() {
        int last_count = 0;
        while (running.load()) {
            std::this_thread::sleep_for(std::chrono::seconds(2));  // 2초마다
            int current = frame_count.load();
            int diff = current - last_count;
            double fps = diff / 2.0;
            std::cout << "[STATS] Total: " << current 
                      << ", Recent FPS: " << std::fixed << std::setprecision(1) << fps 
                      << ", Running: " << (running.load() ? "YES" : "NO") << std::endl;
            last_count = current;
        }
    });
